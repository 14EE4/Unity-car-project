...
- 2026-05-17: **메인 메뉴 키 가이드 패널 - 씬 전환 후 표시 안 되는 문제 해결**
  - **문제**: 게임 진입 후 메인 메뉴로 돌아온 후 키 가이드 버튼을 클릭해도 패널이 나타나지 않음
  - **원인**: `PauseMenuController.CloseKeyGuide()`에서 `GameObject.Destroy(overlay)`로 KeyGuideOverlay를 **영구 삭제**
    - 게임 중 ESC → Pause Menu에서 Key Guide를 닫으면 Overlay가 완전히 파괴됨
    - 메인 메뉴로 복귀 후 Key Guide 버튼 클릭 시 KeyGuidePanel은 있지만 Overlay가 없어서 표시되지 않음
  - **해결 방법**:
    1. `PauseMenuController.CloseKeyGuide()` 수정
       - `GameObject.Destroy(overlay)` → `overlay.SetActive(false)` 변경 (영구 삭제 아닌 비활성화)
    2. `MainMenuController.CloseKeyGuide()` 수정
       - 동일하게 `GameObject.Destroy()` → `SetActive(false)` 적용
