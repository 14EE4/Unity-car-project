...
## 남은 작업 (문서 기반 상세 할 일)
다음은 프로젝트 계획서에 따라 우선순위와 일정별로 정리한 세부 작업 목록입니다.

- **단기 (즉시 — 2주)**
  - 차량 기본 주행 안정화: 가속/제동/조향 입력 흐름 최종 점검
  - 엔진 토크·기어비 기반 물리 로직 보강 (`WheelCollider` 튜닝)
  - 입력 분리/고정확도: 마우스 X축 조향 + 키보드(W/S) 페달 동작 확인
  - 기어 전환 규칙 구현 테스트(1→N→R 전환 포함)

- **중기 (2주 — 4주)**
  - 하이브리드 입력 정교화: 마우스 민감도, 데드존, 키보드 반응성 튜닝
  - 수동 변속 시스템 완성(숫자키로 기어 업/다운)
  - 1인칭/3인칭 카메라 전환 구현 및 카메라 보정
  - 레이스 플로우 기본 구현: 신호등 카운트다운, 체크포인트 순서 판정
  - ESC 일시정지 메뉴(메인 메뉴 복귀/종료) 기본 동작

- **장기 (4주 — 최종)**
  - 트랙 확정 및 주행 테스트, 트랙 이탈 시 노면 마찰 감소(슬립) 처리 고도화
  - UI 구현: 속도, 현재 기어, 조향 표시 바, 랩 타임 표시
  - 랩 타임 측정·저장·조회 시스템(파일 또는 PlayerPrefs 기반)
  - 음향 적용: 엔진 RPM 연동 피치 변화, 스키드/환경음 추가
  - 최종 밸런싱: 조작감, 가속/제동/마찰/사운드 튜닝
  - 통합 테스트 및 예외 처리(체크포인트 누락, 타이머 일시정지 등)

- **개발/배포 관련 작업**
  - 에셋 최적화: 불필요 리소스 제거, URP 설정·머티리얼 최적화
  - 빌드 생성 및 성능 프로파일링
  - 시연 동영상 및 문서(제출용) 정리

각 항목을 완료할 때마다 이 `README.md`의 해당 항목에 체크(또는 업데이트)를 추가하고, 필요 시 관련 스크립트/씬 파일 경로를 기록해 주세요.
